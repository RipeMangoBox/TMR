...
RUN_DIR_OVERRIDE="${RUN_DIR_OVERRIDE:-RUN_DIR/stage4_1_realdata_d2b}"
...
if ! "${CONDA_BIN}" --version >/dev/null 2>&1; then
  echo "[D2b realdata] ERROR: unable to resolve conda executable (${CONDA_BIN})" >&2
  exit 1
fi
...
echo "[D2b realdata] conda: ${CONDA_BIN}"
