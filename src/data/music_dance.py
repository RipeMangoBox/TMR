...
def load_music_feats(path):
    feats = {}
    for file in os.listdir(path):
        if file.endswith(".npy"):
            keyid = file.split(".npy")[0]
            feats[keyid] = torch.from_numpy(np.load(os.path.join(path, file)))
    return feats
...
class MusicDanceDataset(Dataset):
    ...
    def load_keyid(self, keyid):
        # The annotations includes multiple annotation for each motion sequence in hml3d, but one to one for finedance
