...
def compute_global_positions(cfg: DictConfig):
    base_folder = cfg.base_folder
    output_folder = cfg.video_folder    #TODO, original: cfg.output_folder
    force_redo = cfg.force_redo

    from src.guofeats import get_global_positions
    from .tools import loop_amass
    ...
    for motion_path, new_motion_path in iterator:
        ...
        # apply transformation
        try:
            gpositions = get_global_positions(joints)
            gpositions_m = get_global_positions(joints_m)
        except (IndexError, ValueError):
            # The sequence should be only 1 frame long
            # so we cannot compute features (which involve velocities etc)
            assert len(joints) == 1
            continue
        # save the features
        np.save(new_motion_path, gpositions)
